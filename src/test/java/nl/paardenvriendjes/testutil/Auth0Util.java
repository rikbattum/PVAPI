...
public class Auth0Util {
	...
	public String login(String username, String password)
		...
		URIBuilder ub = new URIBuilder("https://pvapp.eu.auth0.com/oauth/ro");
		// ub.setParameter("", "");
		URI uri = ub.build();
		...
		HttpUriRequest request = org.apache.http.client.methods.RequestBuilder.post().setUri(uri)
				.setHeader(HttpHeaders.ACCEPT, " text/plain").setEntity(new UrlEncodedFormEntity(urlParameters))
				.build();
		...
		// Verify access_token
		ub = new URIBuilder("https://pvapp.eu.auth0.com/userinfo");
		uri = ub.build();
		request = RequestBuilder.get().setUri(uri).setHeader(HttpHeaders.AUTHORIZATION, "Bearer " + access_token)
				.build();
		...
		// verify id_token
		ub = new URIBuilder("https://pvapp.eu.auth0.com/tokeninfo");
		uri = ub.build();

		urlParameters = new ArrayList<NameValuePair>();
		urlParameters.add(new BasicNameValuePair("id_token", id_token));
		request = RequestBuilder.post().setUri(uri).setEntity(new UrlEncodedFormEntity(urlParameters)).build();
		response = client.execute(request);
		status = response.getStatusLine().getStatusCode();
		if (status != 200) {
			throw new ClientProtocolException("Unexpected response status: " + status);
	...
	public void logout() throws URISyntaxException, ClientProtocolException, IOException, JSONException {
		URIBuilder ub = new URIBuilder("https://pvapp.eu.auth0.com/v2/logout?returnTo=https://www.google.com");
		// ub.setParameter("", "");
		URI uri = ub.build();
